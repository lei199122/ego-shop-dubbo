...
export default {
  ...
  methods: {
    init (areaId, dataMenu) {
      this.dataForm.areaId = areaId || 0
      this.visible = true
      this.selectedOptions = []
      this.areaList = dataMenu
      this.$nextTick(() => {
        this.$refs['dataForm'].resetFields()
        if (this.dataForm.areaId) {
          this.$http({
            url: this.$http.adornUrl('/shop/area/info/' + this.dataForm.areaId),
            method: 'get',
            ...
            // idList()代表我从父菜单里面选择我的父亲->父亲->xx->xx
            this.selectedOptions = idList(this.areaList, this.dataForm.parentId, 'areaId', 'children').reverse()
            //  this.selectedOptions 代表了我选择的
    ...
    dataFormSubmit () {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl('/shop/area'),
